#!/usr/bin/env pwsh
# Build Inventory Bridge and package a Nexus-ready zip under dist/.
# Usage:  ./pack.ps1            (Release — bundles the ShipOptimizer web UI)
#         ./pack.ps1 -Configuration Debug   (no web UI)
# The zip has BepInEx\ at its root (BepInEx\plugins\InventoryBridge\ + ui) so it installs via a mod
...
[xml]$proj = Get-Content "$root/InventoryBridge.csproj"
...
dotnet build "$root/InventoryBridge.csproj" -c $Configuration

$dist  = "$root/dist"
$stage = "$dist/BepInEx/plugins/InventoryBridge"
Remove-Item $dist -Recurse -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Force -Path $stage | Out-Null
Copy-Item "$root/bin/$Configuration/InventoryBridge.dll" $stage
...
$zip = "$dist/InventoryBridge-v$ver.zip"
